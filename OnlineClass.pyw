...
root = tk.Tk()
root.bind("<Command-w>", lambda x: close())  # Close OCH on pressing Control + W
root.iconbitmap("images/OCH_Logo.icns")
...
def notify(title, content, link, automatic):
    titlePart = ''
    if (not title is None):
        titlePart = 'with title "{0}"'.format(title)
    contentPart = ''
    if (not content is None):
        contentPart = 'subtitle "{0}"'.format(content)

    appleScriptNotification = 'display notification "{0}" {1} {2}'.format("Online Class Helper", titlePart, contentPart)
...
combo2.place(relx=0.59, rely=0.12, relwidth=0.165)
...
combo3.place(relx=0.175, rely=0.325, relwidth=0.05)
...
label16.place(relx=0.23, rely=0.3)
